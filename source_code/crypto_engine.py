...
def loadKey(keyString:str,characterSet:tuple):
    ...
    if(not (" | " in keyString)):
        return (False, "input error: (error 42) integrity check 0 failed; could not find separator 1. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(len(keyList)<5):
        return (False, "input error: (error 43) integrity check 1 failed; key data is incomplete. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")


    #integrity check 2 (has a numeric part count)
    if(not keyList[0].isnumeric()):
        return (False, "input error: (error 44) integrity check 2 failed; key data has invalid complexity value. given key \ndata is invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(key[0]<1):
        return (False, "input error: (error 45) integrity check 3 failed; key data has invalid complexity value. given key \ndata is invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(len(keyList)!=(key[0]*2)+3):
        ...
        return (False, "input error: (error 46) integrity check 4 failed; key data is improperly partitioned. given key data \nis invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    for rotor in range(key[0]):#key 0 is the number of rotors
        ...
        if(not ("," in keyList[rotor+1])):
            return (False, "input error: (error 47) integrity check 5 failed; could not find separator 2. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")
        ...
        for character in range(len(key[rotor+1])):
            ...
            if(not (key[rotor+1][character].isnumeric())):
                return (False, "input error: (error 49) integrity check 7 failed; invalid values found in key data. given key data\nis invalid and possibly corrupted. please check the key for \nerrors then try again.")
            ...
            if(not(key[rotor+1][character]>=0 and key[rotor+1][character]<characterSetLen)):
                return (False, "input error: (error 50) integrity check 8 failed; out of range values found in key data. \ngiven key data is invalid and possibly corrupted. please check the key for \nerrors then try again.")
    ...
    for rotor in range(key[0]):
        ...
        if(not (keyList[rotor+1+key[0]].isnumeric())):
            return (False, "input error: (error 51) integrity check 9 failed; invalid values found in key data. given key \ndata is invalid and possibly corrupted. please check the key for \nerrors then try again.")
        ...
        if(not ((key[rotor+1+key[0]]>=0) and (key[rotor+1+key[0]]<characterSetLen))):
            return (False, "input error: (error 52) integrity check 10 failed; out of range values found in key data. given \nkey data is invalid and possibly corrupted. please check the key \nfor errors then try again.")
    ...
    for cypher in range(2):#same as rotor compilation
        ...
        if(not ("," in keyList[cypher+1+(key[0]*2)])):
            return (False, "input error: (error 53) integrity check 11 failed; could not find separator 3. given key data \nis invalid and possibly corrupted. please check the key for \nerrors then try again.")
        ...
        for character in range(len(key[cypher+1+(key[0]*2)])):
            ...
            if(not (key[cypher+1+(key[0]*2)][character].isnumeric())):
                return (False, "input error: (error 55) integrity check 13 failed; invalid values found in key data. given key \ndata is invalid and possibly corrupted. please check the key for \nerrors then try again.")
            ...
            if(not(key[cypher+1+(key[0]*2)][character]>=0 and key[cypher+1+(key[0]*2)][character]<characterSetLen)):
                return (False, "input error: (error 56) integrity check 14 failed; out of range values found in key data. \ngiven key data is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(key[0]):
        if(type(key[check+1])!=list):
            return (False, "input error: (error 57) integrity check 15 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(key[0]):
        if(type(key[key[0]+check+1])!=int):
            return (False, "input error: (error 58) integrity check 16 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(2):
        if(type(key[(key[0]*2)+check+1])!=list):
            return (False, "input error: (error 59) integrity check 17 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    if(1+verificationRotorCount+verificationStartPosCount+verificationCypherCount!=len(key)):
        (False, "input error: (error 60) integrity check 18 failed; key data is improperly partitioned. given key data is \ninvalid and possibly corrupted. please check the key for errors then \ntry again.")
