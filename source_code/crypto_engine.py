...
def generateKey(rotorCount:int, characterSet:tuple):
    ...
    for i in range(rotorCount):#generates one position for each of the rotors
        ...
        key.append(randint(0,len(characterSet)-1))#used to be a huge fencepost bug here, but now is fixed
...
def loadKey(keyString:str,characterSet:tuple):
    ...
    if(not (" | " in keyString)):
        return (False, "input error: (error code: E-5-1) integrity check 0 failed; could not find separator 1. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(len(keyList)<5):
        return (False, "input error: (error code: E-5-2) integrity check 1 failed; key data is incomplete. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")


    #integrity check 2 (has a numeric part count)
    if(not keyList[0].isnumeric()):
        return (False, "input error: (error code: E-5-3) integrity check 2 failed; key data has invalid complexity value. given key \ndata is invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(key[0]<1):
        return (False, "input error: (error code: E-5-4) integrity check 3 failed; key data has invalid complexity value. given key \ndata is invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    if(len(keyList)!=(key[0]*2)+3):
        ...
        return (False, "input error: (error code: E-5-5) integrity check 4 failed; key data is improperly partitioned. given key data \nis invalid and possibly corrupted. please check the key for errors then try again.")
    ...
    for rotor in range(key[0]):#key 0 is the number of rotors
        ...
        if(not ("," in keyList[rotor+1])):
            return (False, "input error: (error code: E-5-6) integrity check 5 failed; could not find separator 2. given key data is \ninvalid and possibly corrupted. please check the key for errors then try again.")
        ...
        for character in range(len(key[rotor+1])):
            ...
            if(not (key[rotor+1][character].isnumeric())):
                return (False, "input error: (error code: E-5-8) integrity check 7 failed; invalid values found in key data. given key data\nis invalid and possibly corrupted. please check the key for \nerrors then try again.")
            ...
            if(not(key[rotor+1][character]>=0 and key[rotor+1][character]<characterSetLen)):
                return (False, "input error: (error code: E-5-9) integrity check 8 failed; out of range values found in key data. \ngiven key data is invalid and possibly corrupted. please check the key for \nerrors then try again.")
    ...
    for rotor in range(key[0]):
        ...
        if(not (keyList[rotor+1+key[0]].isnumeric())):
            return (False, "input error: (error code: E-5-10) integrity check 9 failed; invalid values found in key data. given key \ndata is invalid and possibly corrupted. please check the key for \nerrors then try again.")
        ...
        if(not ((key[rotor+1+key[0]]>=0) and (key[rotor+1+key[0]]<characterSetLen))):
            ...
            #error was repaired
            return (False, "input error: (error code: E-5-11) integrity check 10 failed; out of range values found in key data. given \nkey data is invalid and possibly corrupted. please check the key \nfor errors then try again.")
    ...
    for cypher in range(2):#same as rotor compilation
        ...
        if(not ("," in keyList[cypher+1+(key[0]*2)])):
            return (False, "input error: (error code: E-5-12) integrity check 11 failed; could not find separator 3. given key data \nis invalid and possibly corrupted. please check the key for \nerrors then try again.")
        ...
        if(not(len(key[cypher+1+(key[0]*2)])==characterSetLen)):
            return (False, "input error: (error code: E-5-13) integrity check 12 failed; key data is incompatible with currently loaded \ncharacter set and may be corrupted. given key data is invalid and possibly corrupted. \nplease check the key for errors then try again.")

        #convert these strings into integers
        for character in range(len(key[cypher+1+(key[0]*2)])):

            #integrity check 13 (all indexes are numeric)
            if(not (key[cypher+1+(key[0]*2)][character].isnumeric())):#despite what it looks like, this address should be a string of an int at this point
                return (False, "input error: (error code: E-5-14) integrity check 13 failed; invalid values found in key data. given key \ndata is invalid and possibly corrupted. please check the key for \nerrors then try again.")
            ...
            if(not(key[cypher+1+(key[0]*2)][character]>=0 and key[cypher+1+(key[0]*2)][character]<characterSetLen)):
                return (False, "input error: (error code: E-5-15) integrity check 14 failed; out of range values found in key data. \ngiven key data is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(key[0]):
        if(type(key[check+1])!=list):
            return (False, "input error: (error code: E-5-16) integrity check 15 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(key[0]):
        if(type(key[key[0]+check+1])!=int):
            return (False, "input error: (error code: E-5-17) integrity check 16 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    for check in range(2):
        if(type(key[(key[0]*2)+check+1])!=list):
            return (False, "input error: (error code: E-5-18) integrity check 17 failed; key data is improperly formatted. given key \ndata is invalid and possibly corrupted. please check the key for errors then \ntry again.")
    ...
    if(1+verificationRotorCount+verificationStartPosCount+verificationCypherCount!=len(key)):
        (False, "input error: (error code: E-5-19) integrity check 18 failed; key data is improperly partitioned. given key data is \ninvalid and possibly corrupted. please check the key for errors then \ntry again.")
...
def advanceRotors(rotorList:list[Rotor]):
    rollover=True
